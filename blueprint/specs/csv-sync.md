...
Module nhập dữ liệu sinh viên từ file **CSV** được hệ thống quản lý cũ của trường export. Hệ thống cũ không có API — chỉ có thể đọc CSV theo lịch cố định. UniHub Workshop chạy **cron job mỗi 15 phút** để nhập dữ liệu và **tạo tài khoản sinh viên tự động** với mật khẩu mặc định `{MSSV}@unihub`. Đây là nguồn duy nhất để tạo tài khoản sinh viên — hệ thống **không hỗ trợ tự đăng ký (self-registration)**.

## Actor

| Actor | Vai trò |
|-------|---------|
| System (Cron) | Tự động trigger import mỗi 15 phút |
| Legacy Student System | Export CSV vào `/data/import/` hàng đêm |
| ORGANIZER | Xem log kết quả import, trigger import thủ công |

## Luồng chính

### Luồng 1 — Import tự động (Cron mỗi 15 phút)

1. Cron job trigger **mỗi 15 phút** (`*/15 * * * *`).
...
5. Queue worker xử lý:
   ...
   c. **Upsert** theo `student_id`:
      - Nếu chưa tồn tại → INSERT (tạo user mới, role = STUDENT, mật khẩu mặc định `{MSSV}@unihub`).
...
- **Mật khẩu mặc định** cho SV mới: hash của `{MSSV}@unihub` (VD: `SV001@unihub`).
