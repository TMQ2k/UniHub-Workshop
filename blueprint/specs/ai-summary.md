...
Module cho phép ban tổ chức upload file **PDF** giới thiệu workshop. Hệ thống tự động xử lý: tách nội dung, làm sạch văn bản, gửi sang **Google Gemini API** để tạo bản tóm tắt hiển thị trên trang chi tiết workshop.
...
| Google Gemini API (External) | Xử lý nội dung, tạo tóm tắt |
...
7. Queue worker xử lý:
   ...
   d. Gọi **Google Gemini API** (model `gemini-2.0-flash`) với prompt tóm tắt tiếng Việt.
...
| Gemini API timeout | Retry 2 lần, nếu vẫn lỗi → mark FAILED | `AI_TIMEOUT` | — |
| Gemini API key invalid | Log error, alert admin | `AI_AUTH_FAILED` | — |
...
- API key lưu trong biến môi trường `GOOGLE_AI_API_KEY`, không hardcode.
...
- [ ] Gemini API lỗi → retry 3 lần, nếu vẫn lỗi → mark FAILED.
